...
class TestXonv1D(unittest.TestCase):
    def setUp(self):
        self.batch_size = 2
        self.in_channels = 3
    ...
    def test_same_kernels(self):
        """Test if Xonv1D produces the same output as nn.Conv1d when all kernels
        are the same."""
        with torch.no_grad():
            self.xonv.weights.data.copy_(
                self.standard_conv.weight.data.unsqueeze(0).expand(
                    self.input_size,
                    -1,
                    -1,
                    -1,
            ...
            self.xonv.bias.data.copy_(
                self.standard_conv.bias.data.unsqueeze(0).expand(
                    self.input_size,
                    -1,
                )
            )
    ...
    def test_different_kernels(self):
        """Test if Xonv1D produces different outputs for different spatial
        locations."""
        output1 = self.xonv(self.input_tensor)

        # Modify weights at a specific location
        with torch.no_grad():
            self.xonv.weights.data[0] *= 2
    ...
    def test_stride_3(self):
        ...
        expected_shape = (
            self.batch_size,
            self.out_channels,
            math.ceil(self.input_size / stride),
        )
        self.assertEqual(output.shape, expected_shape)

    def test_stride_same_kernels(self):
        """Test if Xonv1D with stride produces the same output as nn.Conv1d with
        stride when all kernels are the same."""
        stride = 2
        standard_conv_stride = nn.Conv1d(
            self.in_channels,
            self.out_channels,
            self.kernel_size,
            padding=self.kernel_size // 2,
        ...
        with torch.no_grad():
            xonv_stride.weights.data.copy_(
                standard_conv_stride.weight.data.unsqueeze(0).expand(
                    math.ceil(self.input_size / stride),
                    -1,
                    -1,
                    -1,
            ...
            xonv_stride.bias.data.copy_(
                standard_conv_stride.bias.data.unsqueeze(0).expand(
                    math.ceil(self.input_size / stride),
                    -1,
                )
            )
    ...
    def test_non_divisible_input_size(self):
        ...
        stride = 3
        input_size = 127  # Not divisible by 3
