...
class Xonv2D(nn.Module):
    ...
    def __init__(
        ...
        torch.nn.init.kaiming_uniform_(self.weights, a=1.0)

    def __repr__(self):
        """String representation of the layer."""
        return (
            f"Xonv2D(in_channels={self.in_channels}, "
...
class Xonv1D(nn.Module):
    """
    A 1D convolutional layer with spatially varying kernel and stride support.

    This layer performs a 1D convolution operation where each spatial location
    in the input has its own unique convolutional kernel.
    ...
    Args:
        ...
        out_channels (int): Number of channels produced by the convolution
        kernel_size (int): Size of the convolving kernel
        input_size (int): Length of the input tensor
        stride (int): Stride of the convolution. Default: 1

    Shape:
        - Input: (batch_size, in_channels, length)
        - Output: (batch_size, out_channels, length // stride)

    Examples:
        >>> input_size = 128
        >>> layer = Xonv1D(3, 16, 5, input_size, stride=2)
        >>> input_tensor = torch.randn(1, 3, 128)
        >>> output = layer(input_tensor)
        >>> print(output.shape)
        torch.Size([1, 16, 64])
    """
    ...
    def __init__(
        ...
        # Calculate output size
        self.output_size = math.ceil(input_size / stride)

        # Create a weight tensor for each spatial location in the output.
        # Shape: (output_length, out_channels, in_channels, kernel_size)
        self.weights = nn.Parameter(
            torch.randn(
                self.output_size,
                out_channels,
        ...
        # Create a bias tensor for each spatial location in the output
        # Shape: (output_length, out_channels)
        self.bias = nn.Parameter(
            torch.randn(
                self.output_size,
                out_channels,
    ...
    def __repr__(self):
        """String representation of the layer."""
        return (
            f"Xonv1D(in_channels={self.in_channels}, "
            f"out_channels={self.out_channels}, "
            f"kernel_size={self.kernel_size}, "
            f"input_size={self.input_size}, "
            f"stride={self.stride})"
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            torch.Tensor: Output tensor of shape (batch_size, out_channels, length // stride)
        """
        ...
        # Pad the input tensor
        if self.kernel_size % 2 == 1:
            x_padded = F.pad(x, (pad, pad))
        else:
            x_padded = F.pad(x, (pad - 1, pad))

        # Extract patches from the padded input with stride.
        # Shape: (batch_size, length // stride, in_channels, kernel_size)
        ...
        # Perform batch matrix multiplication
        # Shape: (batch_size, output_length, out_channels)
        output = torch.matmul(
            patches.unsqueeze(2),
...
if __name__ == "__main__":
    input_size = 128  # Length of input
    in_channels = 3
    out_channels = 16
    kernel_size = 5
    stride = 2
    ...
    print(layer)
    print(output.shape)  # Should be [1, 16, 64]
